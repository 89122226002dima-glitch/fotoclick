...
// Возвращаем вызовы API на внутренний бэкенд Vercel
...
async function generate() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    placeholder.className = 'bg-[#353739] rounded-lg flex items-center justify-center relative overflow-hidden aspect-square';
  ...
  if (progressContainer && progressBar && progressText) {
      ...
      progressText.innerText = 'Подготовка... 0%';
  }
  ...
  try {
    ...
    for (let i = 0; i < generationPrompts.length; i++) {
        ...
        try {
            if (progressContainer && progressBar && progressText) {
              const progress = ((i + 1) / generationPrompts.length) * 100;
              progressBar.style.width = `${progress}%`;
              progressText.innerText = `Генерация ${i + 1} из 4... ${Math.round(progress)}%`;
            }

            const imageUrl = await generateVariation(prompt, referenceImage!);
            ...
            imgContainer.addEventListener('click', e => { if (!(e.target as HTMLElement).closest('a, button')) openLightbox(img.src); });
             ...
             // If one image fails, show an error in its placeholder and continue
             const errorMessage = e instanceof Error ? e.message : 'Ошибка генерации';
             displayErrorInContainer(placeholders[i], errorMessage, true);
             // We still consumed a credit for the attempt, so we don't refund it here,
             // but we do need to stop the whole process if a critical error occurs.
             // For this sequential approach, we'll log it and let it finish.
...
function setupNavigation() {
    ...
    const navigateToPage = (pageId: string) => {
        ...
        document.querySelector<HTMLElement>(`#${pageId}`)?.classList.remove('hidden');
    ...
    const handleFile = (file: File) => {
        ...
        reader.onload = async (e) => {
            ...
            if (dataUrl) {
                ...
                const mimeType = header.match(/:(.*?);/)?.[1] || 'image/png';
                await onStateChange({ base64, mimeType });
            }
        };
...
async function analyzeImageForText(image: ImageState, analysisPrompt: string): Promise<string> {
    try {
        const data = await apiRequest('analyzeImageForText', { image, analysisPrompt });
        return data.text;
    } catch (e) {
...
function initializePage1Wizard() {
    ...
    const generatePhotoshoot = async () => {
        ...
        photoshootResultContainer.innerHTML = `<div class="loading-spinner flex flex-col items-center justify-center" role="status">
            <p id="photoshoot-loading-text" class="text-white text-lg mt-4">Подготовка фотосессии...</p>
        ...
        try {
            ...
            if (page1ClothingImage) {
                ...
                const additionalClothingDetails = clothingText ? ` Дополнительные пожелания к одежде (например, изменение цвета): "${clothingText}".` : '';
                promptText = `Твоя задача — провести цифровую фотосессию. Тебе даны два изображения.
Первое изображение — это РЕФЕРЕНС ЧЕЛОВЕКА, который задает формат итогового фото.
Второе изображение — это РЕФЕРЕНС ОДЕЖДЫ.
КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
1.  **СОХРАНИ ЧЕЛОВЕКА (из первого фото):** Возьми человека с ПЕРВОГО изображения. Его внешность, уникальные черты лица (форма носа, глаз, губ), цвет кожи, прическа, выражение лица и поза должны остаться АБСОЛЮТНО ИДЕНТИЧНЫМИ оригиналу. Это самое важное правило. Не изменяй человека.
2.  **СОХРАНИ ФОРМАТ И КАДРИРОВАНИЕ:** Итоговое изображение должно иметь ТОЧНО ТАКОЕ ЖЕ соотношение сторон (формат), композицию и кадрирование, как у ПЕРВОГО фото (референса человека). Второе фото (одежда) используется только как источник стиля одежды, его формат и композицию нужно полностью игнорировать.
3.  **ПРИМЕНИ ОДЕЖДУ (из второго фото):** Возьми одежду, показанную на ВТОРОМ изображении, и надень ее на человека из первого изображения. Одежда должна быть перенесена МАКСИМАЛЬНО ТОЧНО, включая фасон, цвет, текстуру и принт. Если на втором фото тоже есть человек, ИГНОРИРУЙ ЕГО ПОЛНОСТЬЮ, используй только одежду.${additionalClothingDetails}
4.  **ЗАМЕНИ ФОН:** Полностью замени оригинальный фон на новый: "${locationText}".
5.  **АДАПТИРУЙ ОСВЕЩЕНИЕ:** Сделай так, чтобы освещение на человеке и новой одежде гармонично соответствовало новому фону. Освещение должно выглядеть естественно и фотореалистично.
Результат — только одно изображение без текста.`;
            } else {
                promptText = `Твоя задача — действовать как 'цифровой стилист', используя это референсное фото.
Твоя главная цель — идеально сохранить человека с фото, изменив только его одежду и фон.
КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
...
2.  **СОХРАНИ КОМПОЗИЦИЮ:** Поза, кадрирование и масштаб исходного фото должны остаться без изменений.
3.  **ЗАМЕНИ ОДЕЖДУ:** Переодень человека в: "${clothingText}". Нарисуй только ту часть одежды, которая видна в кадре.
...
Результат — только одно изображение без текста.`;
    ...
    const updateGenerateButtonState = () => {
        generatePhotoshootButton.disabled = !(page1ReferenceImage && (page1ClothingImage || clothingPromptInput.value.trim()) && locationPromptInput.value.trim() && generationCredits > 0);
    };
    ...
    generatePhotoshootButton.addEventListener('click', generatePhotoshoot);
    clothingPromptInput.addEventListener('input', updateGenerateButtonState);
    locationPromptInput.addEventListener('input', updateGenerateButtonState);
...
function getUploaderPlaceholderHtml(): string {
  return `<div class="w-full h-full flex flex-col items-center justify-center p-4 gap-4"><div class="w-full max-w-xs aspect-square border border-white/60 rounded-lg flex items-center justify-center p-2"><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="1024pt" height="1024pt" viewBox="0 0 1024 1024" preserveAspectRatio="xMidYMid meet" class="w-full h-full object-contain text-white opacity-80 pointer-events-none"><g transform="translate(0,1024) scale(0.1,-0.1)" fill="currentColor" stroke="none"><path d="M4753 9900 c-140 -19 -330 -99 -472 -200 -83 -59 -227 -193 -273 -255 -17 -22 -5 -12 26 22 32 34 93 92 136 127 311 257 650 355 876 252 60 -28 65 -17 6 13 -75 38 -193 54 -299 41z"/><path d="M5235 9197 c-46 -48 -79 -101 -180 -288 -83 -154 -169 -276 -274 -390 -68 -73 -84 -86 -113 -87 -63 -2 -159 -47 -215 -101 -36 -34 -27 -35 22 -1 49 34 115 60 149 60 17 -1 7 -14 -47 -65 -106 -99 -283 -230 -498 -367 -271 -173 -416 -282 -545 -412 -121 -121 -196 -225 -254 -350 -50 -108 -70 -190 -77 -316 -8 -142 13 -222 118 -445 45 -97 83 -174 85 -172 2 2 -28 76 -66 164 -86 197 -110 286 -110 408 0 119 26
 110 -19 156 -1 142 -18 309 -40 378 -55 179 -200 303 -459 392 -116 40 -146 40 -55 0 205 -91 265 -125 338 -194 80 -77 127 -152 146 -240 7 -30 16 -149 20 -265 6 -171 11 -220 28 -265 26 -75 59 -137 94 -182 l28 -36 -162 -80 c-266 -129 -614 -336 -778 -462 -19 -15 -48 -35 -63 -45 l-29 -18 7 194 c4 107 9 212 11 234 12 131 15 629 5 761 -37 466 -69 573 -214 717 -74 73 -168 140 -290 207 -64 35 -89 43 -140 46 -34 1 -71 1 -82 -2z m-872 -2774 c14 -41 76 -185 137 -319 611 -241 111 -244 92 -263 -19 -19 -20 -18 -75 129 -67 176 -175 476 -188 517 -18 59 9 9 34 -64z"/><path d="M5282 3060 c0 -14 2 -19 5 -12 2 6 2 18 0 25 -3 6 -5 1 -5 -13z"/><path d="M5322 2580 c0 -14 2 -19 5 -12 2 6 2 18 0 25 -3 6 -5 1 -5 -13z"/><path d="M5332 2470 c0 -14 2 -19 5 -12 2 6 2 18 0 25 -3 6 -5 1 -5 -13z"/><path d="M4065 2325 c34 -109 49 -199 48 -300 0 -121 -10 -161 -70 -280 -74 -145 -132 -212 -472 -535 -278 -265 -438 -470 -570 -729 -64 -126 -85 -192 -26 -82 75 140 317 425 460 544 113 94 445 399 506 466 214 233 287 499 205 745 -22 66 -98 227 -81 171z"/><path d="M5350 2278 c-1 -36 57 -172 166 -393 139 -283 166 -354 265 -720 98 -357 171 -607 176 -601 5 8 -219 891 -258 1004 -23 64 -70 176 -105 247 -126 258 -243 480 -244 463z" /></g></g></svg></div><div class="text-center"><div class="bg-black/40 backdrop-blur-md p-4 rounded-xl inline-block"><p class="text-gray-200 font-semibold text-lg mb-1">Загрузите ваше фото</p><p class="text-sm max-w-xs mx-auto mb-3 text-gray-400">Для достижения наилучших результатов используйте качественный портрет снятый от талии или бедер и выше, или селфи.</p><div class="p-2 bg-gray-800/50 border border-gray-600/80 rounded-lg transition-colors duration-200 inline-block"><p class="text-gray-200 text-xs font-medium">Нажмите или перетащите файл</p><p class="text-xs text-gray-500 mt-1">PNG, JPG, WEBP</p></div></div></div></div>`;
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const handlePage2Upload = (file: File) => {
      ...
      reader.onload = async (e) => {
        ...
        if (dataUrl) {
          const [header, base64] = dataUrl.split(',');
          const mimeType = header.match(/:(.*?);/)?.[1] || 'image/png';
          const imageState = { base64, mimeType };
          ...
          } catch (e) { showStatusError(e instanceof Error ? e.message : 'Неизвестная ошибка анализа.'); resetApp(); }
    ...
    ['dragover', 'dragleave', 'drop'].forEach(eventName => uploadContainer.addEventListener(eventName, e => {
        ...
        if (eventName === 'dragover') uploadContainer.classList.add('border-blue-500', 'bg-gray-800');
        if (eventName === 'dragleave' || eventName === 'drop') uploadContainer.classList.remove('border-blue-500', 'bg-gray-800');
