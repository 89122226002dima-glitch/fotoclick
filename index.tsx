...
4.  **НОВЫЙ ФОРМАТ ИЗОБРАЖЕНИЯ:** Итоговое изображение должно иметь стандартное фотографическое соотношение сторон **${aspectRatioInstruction}**. Полностью игнорируй оригинальные пропорции обоих загруженных изображений.
5.  **АДАПТИРУЙ КОМПОЗИЦИЮ:** Сохрани основную композицию и кадрирование человека с **Изображения 1** (например, если это был портрет по пояс, результат тоже должен быть портретом по пояс), но адаптируй его под новое соотношение сторон ${aspectRatioInstruction}.
6.  **ЗАМЕНИ ФОН:** Полностью замени оригинальный фон на новый: "${locationText}".
7.  **АДАПТИРУЙ ОСВЕЩЕНИЕ:** Сделай так, чтобы освещение на человеке и новой одежде гармонично соответствовало новому фону, не искажая при этом черты лица или цвет кожи человека с Изображения 1.
