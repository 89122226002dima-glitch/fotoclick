...
async function generate() {
  ...
  try {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        let finalPrompt = `Это референсное фото. Твоя задача — сгенерировать новое фотореалистичное изображение.\n\n`;
        
        if (referenceFaceImage) {
             finalPrompt += `ВТОРОЕ ИЗОБРАЖЕНИЕ (крупный план) — ЭТО ЭТАЛОН ЛИЦА (Face ID). Используй его как главный источник информации о внешности.\n\n`;
        }

        finalPrompt += `КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:\n`;
        finalPrompt += `1.  **ПРИОРИТЕТ — ИДЕНТИЧНОСТЬ:** Лицо на новом фото должно быть **100% узнаваемым**. Форма носа, разрез глаз, губы, овал лица и уши должны быть скопированы с ${referenceFaceImage ? 'ЭТАЛОНА (второго фото)' : 'оригинала'} с максимальной точностью.\n`;
        finalPrompt += `2.  **НОВАЯ КОМПОЗИЦИЯ:** Примени следующие изменения: "${changesDescription}".\n`;
        finalPrompt += `3.  **СОХРАНИ ОДЕЖДУ:** Одежда человека должна быть взята с ${referenceFaceImage ? 'первого ' : ''}референсного фото.\n`;
        finalPrompt += `${backgroundPromptPart}\n`;
        
        if (customText) {
            finalPrompt += `5. **ВАЖНОЕ ДОПОЛНЕНИЕ:** ${customText}\n`;
        }

        finalPrompt += `\n6.  **ЗАПРЕТ НА "УЛУЧШАЙЗИНГ" (NO BEAUTY FILTER):**
    *   **Запрещено** сглаживать кожу или делать "бьюти-ретушь".
    *   **ОБЯЗАТЕЛЬНО СОХРАНИ** естественную текстуру кожи, микро-детали, родинки, морщинки, носогубные складки и индивидуальные особенности возраста.
    *   Человек должен выглядеть живым и настоящим, а не как "отфотошопленная модель". Реализм важнее красоты.\n`;

        finalPrompt += `\n7.  **ЕСТЕСТВЕННОЕ ОСВЕЩЕНИЕ:** Используй мягкий, естественный свет, который подчеркивает реальный объем лица, не искажая черты (избегай слишком жесткого контраста, который меняет восприятие лица).\n`;

        finalPrompt += `\n**КАЧЕСТВО:** Высокая детализация, фотореализм.\n\nРезультат — только одно изображение без текста.`;
...
1.  **СОХРАНИ ЧЕЛОВЕКА:** Внешность, уникальные черты лица (форма носа, глаз, губ), цвет кожи, прическа и выражение лица человека с ПЕРВОГО фото должны остаться АБСОЛЮТНО ИДЕНТИЧНЫМИ оригиналу. **Поза и выражение лица ОБЯЗАТЕЛЬНО должны остаться без изменения.** Это самое важное правило.
...
5.  **АДАПТИРУЙ ОСВЕЩЕНИЕ:** Сделай так, чтобы освещение на человеке гармонично соответствовало новому фону, но при этом НЕ ИЗМЕНЯЙ его черты лица. Освещение должно выглядеть естественно и фотореалистично.
...
*   **Текстура:** Добавь едва заметное пленочное зерно (film grain) для реалистичности. **Кожа должна быть безупречно гладкой, сияющей и здоровой, с легким эффектом профессиональной бьюти-ретуши. Полностью исключи пигментные пятна, рыхлость и лишние возрастные детали, сохраняя при этом естественность. Лицо должно выглядеть свежим, молодым и ухоженным.**
