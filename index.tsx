...
2.  **АДАПТИРУЙ КОМПОЗИЦИЮ:** Сохрани основную композицию и кадрирование человека с референсного фото (например, если это был портрет по пояс, результат тоже должен быть портретом по пояс), но адаптируй его под новое соотношение сторон ${aspectRatioInstruction}. Поза и лицо ОБЯЗАТЕЛЬНО должны остаться без изменения.
3.  **НОВЫЙ ФОРМАТ ИЗОБРАЖЕНИЯ:** Итоговое изображение должно иметь стандартное фотографическое соотношение сторон **${aspectRatioInstruction}**. Игнорируй оригинальные пропорции референсного фото.
