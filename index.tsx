...
Изображение 1 — это РЕФЕРЕНС ЧЕЛОВЕКА.
Изображение 2 — это РЕФЕРЕНС ОДЕЖДЫ (может быть скриншотом).

КРИТИЧЕСКИ ВАЖНЫЕ, НЕУКОСНИТЕЛЬНЫЕ ПРАВИЛА:
1.  **АБСОЛЮТНАЯ ИДЕНТИЧНОСТЬ ЧЕЛОВЕКА:** Человек на итоговом фото должен быть точной, 1-в-1 копией человека с **Изображения 1**. Его лицо, черты лица (нос, глаза, губы), цвет кожи, прическа, выражение лица и поза должны быть сохранены без малейших изменений. Это главное правило.
2.  **ИГНОРИРУЙ ЧЕЛОВЕКА НА ВТОРОМ ФОТО:** На **Изображении 2** (одежда) может быть другой человек. Ты должен **ПОЛНОСТЬЮ ИГНОРИРОВАТЬ** этого человека. Используй с Изображения 2 **ТОЛЬКО И ИСКЛЮЧИТЕЛЬНО ОДЕЖДУ**.
3.  **ПЕРЕНЕСИ ОДЕЖДУ:** Возьми одежду, показанную на **Изображении 2**, и аккуратно "надень" ее на человека с **Изображения 1**. Фасон, цвет, текстура и принт одежды должны быть перенесены максимально точно.${additionalClothingDetails}
4.  **КЛЮЧЕВАЯ ЗАДАЧА - ФОРМАТ И КАДРИРОВАНИЕ:** Итоговое изображение должно в точности повторить **соотношение сторон (формат) и композицию кадра** с **Изображения 1 (референс человека)**. Если второе изображение (скриншот с одеждой) вытянутое по вертикали, твоя задача — **мысленно "обрезать" его** сверху и снизу, чтобы итоговый результат соответствовал формату Изображения 1. Не пытайся вписать человека в вытянутый формат скриншота. Игнорируй пропорции второго изображения полностью.
5.  **ЗАМЕНИ ФОН:** Полностью замени оригинальный фон на новый: "${locationText}".
6.  **АДАПТИРУЙ ОСВЕЩЕНИЕ:** Сделай так, чтобы освещение на человеке и новой одежде гармонично соответствовало новому фону, не искажая при этом черты лица или цвет кожи человека с Изображения 1.

Результат — только одно фотореалистичное изображение без текста.`;
            } else {
