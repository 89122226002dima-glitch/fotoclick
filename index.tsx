...
function setupUploader(containerId: string, inputId: string, previewId: string, placeholderId: string, clearButtonId: string, onStateChange: (state: ImageState | null) => Promise<void>) {
    ...
    if (!uploadContainer || !imageUpload || !imagePreview || !uploadPlaceholder || !clearButton) {
        console.error(`Uploader setup failed: one or more elements not found for container ${containerId}`);
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // <<< START OF REFACTORED PAGE 1 INITIALIZATION >>>
...
Результат — только одно изображение без текста.`;
    ...
    resetWizard();
    // <<< END OF REFACTORED PAGE 1 INITIALIZATION >>>
