...
app.post('/api/generateFourVariations', verifyToken, authenticateAndCharge(4), async (req, res) => {
    ...
    try {
        ...
        if (faceImage) {
            ...
            ВТОРОЕ ИЗОБРАЖЕНИЕ (лицо крупным планом) - это ЭТАЛОН ВНЕШНОСТИ. Используй второе изображение, чтобы максимально точно скопировать черты лица (глаза, нос, губы) на всех вариациях.
